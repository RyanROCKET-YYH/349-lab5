...
FREERTOS_PORT_DIR   		= $(FREERTOS_SRC_DIR)/portable/GCC/ARM_CM4F
FREERTOS_MEM_DIR 	  		= $(FREERTOS_SRC_DIR)/portable/MemMang
OBJ_DIR         			= $(BUILD)/$(PROJ)
OBJ_PROJ_DIR   				= $(OBJ_DIR)/$(PROJ)_$(HASH_PROJ)
PRE_BUILT_OBJ_DIR 			= $(BUILD)/prebuilt
FREERTOS_OBJ_PROJ_DIR 		= $(OBJ_DIR)/$(PROJ)_$(HASH_PROJ)/free-rtos
FREERTOS_PORT_OBJ_PROJ_DIR 	= $(OBJ_DIR)/$(PROJ)_$(HASH_PROJ)/free-rtos/port
FREERTOS_MEM_OBJ_PROJ_DIR 	= $(OBJ_DIR)/$(PROJ)_$(HASH_PROJ)/free-rtos/mem-mang
...
FREERTOS_PORT_SRC 	= $(FREERTOS_PORT_DIR)/port.c
...
FREERTOS_OBJ_RULE 		= $(FREERTOS_SRC:$(FREERTOS_SRC_DIR)/%.c=$(FREERTOS_OBJ_PROJ_DIR)/%.o)
FREERTOS_PORT_OBJ_RULE 	= $(FREERTOS_PORT_SRC:$(FREERTOS_PORT_DIR)/%.c=$(FREERTOS_PORT_OBJ_PROJ_DIR)/%.o)
FREERTOS_MEM_OBJ_RULE 	= $(FREERTOS_MEM_SRC:$(FREERTOS_MEM_DIR)/%.c=$(FREERTOS_MEM_OBJ_PROJ_DIR)/%.o)

# OBJECTS
OBJECTS        			= $(wildcard $(OBJ_PROJ_DIR)/*.o)
PRE_BUILT_OBJECTS 		= $(wildcard $(PRE_BUILT_OBJ_DIR)/*.o)
FREERTOS_OBJECTS  		= $(wildcard $(FREERTOS_OBJ_PROJ_DIR)/*.o)
FREERTOS_PORT_OBJECTS  	= $(wildcard $(FREERTOS_PORT_OBJ_PROJ_DIR)/*.o)
FREERTOS_MEM_OBJECTS  	= $(wildcard $(FREERTOS_MEM_OBJ_PROJ_DIR)/*.o)

# Final output
OUTPUT = $(BIN_DIR)/$(BINARY)

# Path to soft float lib
SOFT_FLOAT_LIB    = $(LIB_DIR)/soft_float/libgcc.a

# FLOAT_ARCH = -fsingle-precision-constant -Wdouble-promotion
# Case on float type, soft by default
ifeq ($(FLOAT), soft)
	LIB_FILES = $(LIB_DIR)/soft_float/libc.a $(SOFT_FLOAT_LIB) -u _printf_float
	FLOAT_ARCH += -mfloat-abi=softfp
else
	LIB_FILES = $(LIB_DIR)/hard_float/libc.a $(LIB_DIR)/hard_float/libm.a  $(SOFT_FLOAT_LIB) -u _printf_float
...
setup:
	...
	$(MKDIR_P) $(FREERTOS_OBJ_PROJ_DIR)
	$(MKDIR_P) $(FREERTOS_PORT_OBJ_PROJ_DIR)
	$(MKDIR_P) $(FREERTOS_MEM_OBJ_PROJ_DIR)
...
$(OBJ_PROJ_DIR)/%.o: $(SRC_DIR)/%.c
	@printf "\n$b$yCompiling: $<$n$n\n" $<
	$(CC) -I$(INC_DIR) -I$(FREERTOS_INC_DIR) -I$(FREERTOS_PORT_DIR) $(CCFLAGS) -c $< -o $@
...
$(FREERTOS_PORT_OBJ_PROJ_DIR)/%.o: $(FREERTOS_PORT_DIR)/%.c
	@printf "\n$b$yCompiling: $<$n$n\n" $<
	$(CC) -I$(INC_DIR) -I$(FREERTOS_INC_DIR) -I$(FREERTOS_PORT_DIR) $(CCFLAGS) -c $< -o $@

$(FREERTOS_MEM_OBJ_PROJ_DIR)/%.o: $(FREERTOS_MEM_DIR)/%.c
	@printf "\n$b$yCompiling: $<$n$n\n" $<
	$(CC) -I$(INC_DIR) -I$(FREERTOS_INC_DIR) -I$(FREERTOS_PORT_DIR) $(CCFLAGS) -c $< -o $@
...
$(BIN_DIR)/$(BINARY).elf: $(K_OBJ_RULE) $(ASM_SRC_RULE) $(FREERTOS_OBJ_RULE) $(FREERTOS_PORT_OBJ_RULE) $(FREERTOS_MEM_OBJ_RULE)
	...
	$(LD) -T /tmp/linker.lds -o $(BIN_DIR)/$(BINARY).elf $(OBJECTS) $(PRE_BUILT_OBJECTS) \
								$(FREERTOS_OBJECTS) $(FREERTOS_PORT_OBJECTS) $(FREERTOS_MEM_OBJECTS) \
								$(LIB_FILES)
